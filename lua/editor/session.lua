...
local function is_startup_target(name)
  for _, path in ipairs(NVEnv.startup.files) do
    if path == name and not NVEnv.completed[path] then
      return true
    end
  end
  return false
...
local function write_snapshot(file, restart)
  ...
  local ok, err = xpcall(function()
    ...
    if vim.fn.writefile(lines, temp) ~= 0 then
      error('Cannot write session: ' .. temp)
    end
...
function M.save()
  if not M.is_saving() or NVEnv.restarting then
    return false
  end
  local named = 0
  ...
  if named == 0 then
    if not NVEnv.workspace_activated then
      return false
    end
    local file = M.current()
    if vim.fn.filereadable(file) == 1 and vim.fn.delete(file) ~= 0 then
      error('Cannot remove empty session: ' .. file)
    end
    return true
  end
  NVClose.consume_all()
...
local function restore_ui(data)
  if data then
    for i, tab in ipairs(vim.api.nvim_list_tabpages()) do
      if data.tabs[i] and data.tabs[i].label then
        vim.t[tab].tab_label = data.tabs[i].label
      end
    end
...
local function load_snapshot(file, restart)
  if not file or vim.fn.filereadable(file) ~= 1 then
    error('Session file is missing: ' .. tostring(file))
  end
  ...
  if data and data.version ~= 1 then
    error('Unsupported session format: ' .. file)
  end
  if restart and not (data and data.context) then
    error('Restart snapshot has no invocation context: ' .. file)
  end
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if vim.bo[buf].modified then
      error 'Save or discard modified buffers before restoring a session'
    end
  end
  ...
  local ok, err = xpcall(function()
    ...
    if data then
        ...
        if path_is_present(item.name) or is_startup_target(item.name) then
          ...
          if item.loaded then
            ...
            if vim.bo[buf].filetype == '' then
              vim.bo[buf].filetype = item.filetype
            end
          end
        end
      end
      ...
      for _, item in ipairs(NVBuffers.get_managed()) do
        if item.name ~= '' and not keep[item.name] then
          delete_session_buffer(item.bufnr)
        end
      end
      NVBuffers.prune_arguments()
    else
      ...
      for _, item in ipairs(NVBuffers.get_managed()) do
        if item.name ~= '' and not path_is_present(item.name) then
          delete_session_buffer(item.bufnr)
        end
      end
    end
...
function M.autocmds()
  vim.api.nvim_create_autocmd('VimLeavePre', {
    ...
    callback = function()
      if NVEnv.restarting or (vim.fn.exists 'v:exitreason' == 1 and tostring(vim.v.exitreason):match '^restart') then
        return
      end
